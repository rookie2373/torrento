...
class TorrentDownload():
    def __init__(self, peer, torrent):
        if DEBUG:
            print(f"[torr_download.py] TorrentDownload initialized for peer {peer.ip}:{peer.port}")
        self.peer = peer
    ...
    def check_block(self, piece_index, chunk_start, payload):
        if self.complete[piece_index]:
            if DEBUG:
                print(f"[torr_download.py] Piece {piece_index} already completed")
        ...
        if DEBUG:
            print(f"[torr_download.py] Piece {piece_index}: {current_length}/{required_len} bytes")
    ...
    def verify_hash(self, piece_index):
        if DEBUG:
            print(f"[torr_download.py] Verifying hash for piece {piece_index}")
        ...
        if file_index_sha != current_piece_sha:
            if DEBUG:
                print(f"[torr_download.py] Hash mismatch for piece {piece_index}")
    ...
    def mark_piece_complete(self, piece_index, current_piece):
        if DEBUG:
            print(f"[torr_download.py] Marking piece {piece_index} as complete")
    ...
    def display_download(self):
        ...
        print('%02.1f%% complete' % up_to_complete)
